...
// MAP
	...
	function drawMap(classes){
		d3.select("#svg").html("");
		 diameter = 800, radius = diameter / 2, innerRadius = radius - 120;
...
/* 	d3.json("hierarchical-edge-bundling-excel", function(error, classes) {
	...
	function filterNodes1() {
		...
		d3.selectAll(".link--source").each(
				...
				d3.selectAll(".link--target").each(
				function(l) {
					console.log(d3.select(this).classed("link--source"))
					//console.log(l.source.data.key)
					count = 0;
					for (j = 0; j < filterdIndex.length; j++) {
						if (l.source.data.key == filterdIndex[j]) {
					...
					if (count == 0) {
						...
						d3.select(this).classed("link--target", false);
					} else {
					console.log(l)
						node.each(function(n) {
					...
					if(d3.select(this).classed("link--source"))
						d3.select(this).classed("link--target", false);
	...
	function filterNodes2() {
		...
		node.each(function(n) {
			n.target = n.source = false;
				...
				link.classed("link--target", function(l) {
					var count =0;
					$.each(filterNode,function(i,d){
						if (l.target === d) {
							count++;
						}
						
					})
					if(count == filterNode.length)
						return l.source.source = true
				}).classed("link--source", function(l) {
					var count =0;
					$.each(filterNode,function(i,d){
