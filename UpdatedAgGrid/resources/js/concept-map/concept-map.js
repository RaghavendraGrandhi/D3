var ConceptMap = function (chartElementId, infoElementId, dataJson) {
	...
	var W = d3.svg.diagonal.radial().projection(function (X) {
		if(!isNaN(X.y) && !isNaN(X.x / 180 * Math.PI )){
			console.log([X.y, X.x / 180 * Math.PI])
	...
	var d = d3.select("#" + chartElementId).html("").append("svg")
		.attr("id", "tempChartID") // added to manage display element in Shiny
		.attr("width", a)
		.attr("height", c)
		.append("g")
		.attr("transform", "translate(" + a / 2 + "," + c / 2 + ")");
	...
	var Q = d3.select("#" + infoElementId).html("")
		.attr("id", "tempElementID"); // added to manage display element in Shiny
		...
		T.get("ditems").forEach(function (ae) {
			ae.links.forEach(function (af) {
				...
				H.push({
					...
					y1: ae.y + K / 2,
					x2: isNaN(Math.cos(Y) * X + ab.xOffset)?0:Math.cos(Y) * X + ab.xOffset,
					y2: isNaN(Math.sin(Y) * X)?0:Math.sin(Y) * X
				})
			})
		});
		P = [];
			...
			tableau.extensions.initializeAsync().then(function () {
			      ...
			      dashboard.worksheets.forEach(function (worksheet) {
			    	  ...
			    	  if(Y == 0){
			    		  ...
			    		  worksheet.getFiltersAsync().then(f=>{
			    			  payLoad.columns.forEach(filter=>{
			    				  let arr = filter.split("_")
			    				  worksheet.clearFilterAsync(arr[1]);
			    			  })
			    			  //worksheet.clearFilterAsync(f);
			    		  })
			    	  }else{
			    		  worksheet.getFiltersAsync().then(f=>{
			    			  f.forEach(filter=>{
			    			  ...
			    			  if(Y.column){
			    				  worksheet.clearFilterAsync(Y.column)
