...
//~ var color = ["#31a354","#1f77b4","#ff7f0e","#000"];
var color = ["#0099ff","#1f77b4","#ff7f0e","#ffffff"];
var raw_chart_width = 2000;

var duration_spacing = 10;
...
function Character_(name, id, group, order, skills, trajectory) {
    this.name = name;
    this.id = id;
    this.group = group;
    this.group_ptr = null;
...
function SceneNode(chars, start, duration, id, minh, maxh) {
    ...
    this.last_scene = null; // Only defined for char_node true
...
function generate_links(chars, scenes) {
	...
	chars[i].last_scene = char_scenes[char_scenes.length-1];
...
function add_char_scenes(chars, scenes, links, groups, panel_shift, comic_name) {
    ...
    chars.sort(function(a, b) { return a.skills[0] - b.skills[0]; });
    for (var i = 0; i < chars.length; i++) {
	var s = new SceneNode([chars[i].id], [0], [1],0,0);
	s.char_node = true;
	s.y = i*text_height; //chars[i].skillorder*text_height;
	s.x = 0;
	s.width = 5;
	s.height = 0;//link_width;
	s.name = chars[i].name;
    ...
    //repeat names on the other side
    chars.sort(function(a, b) { return a.skills[a.skills.length-1] - b.skills[a.skills.length-1]; });
    for (var i = 0; i < chars.length; i++) {
	var s = new SceneNode([chars[i].id], [0], [1],0,0);
	s.char_node = true;
	s.y = i*text_height; //chars[i].skillorder*text_height;
	s.x = raw_chart_width-300;
	s.width = 5;
	s.height = 0;//link_width;
	s.name = chars[i].name;
	s.chars[s.chars.length] = chars[i].id;
	s.id = scenes.length;
	s.comic_name = comic_name;
	if (chars[i].last_scene != null) {
	    var l = new Link(chars[i].last_scene, s, chars[i].group, chars[i].id);
	    l.char_ptr = chars[i];

	    s.in_links[s.in_links.length] = l;
	    chars[i].last_scene.out_links[chars[i].last_scene.out_links.length] = l;
	    links[links.length] = l;
	    s.last_scene = chars[i].last_scene;

	    scenes[scenes.length] = s;
	    //~ char_scenes[char_scenes.length] = s;
	    s.median_group = chars[i].last_scene.median_group;
	} // if
    } // for
    return char_scenes;
} // add_char_scenes
...
function calculate_node_positions(chars, scenes, total_panels, chart_width, 
				  ...
				  panel_shift, char_map,minSkill,maxSkill) {
    ...
    */
    ...
    scenes.forEach(function(scene) {
	if (!scene.char_node) {
	    //~ scene.height = Math.max(0, scene.chars.length*link_width + (scene.chars.length - 1)*link_gap);  // For fixed height scenes change this to a constant.
		...
		//~ var y = c.skills[scene.start]*char_scenes.length*text_height;
		//~ if (!y) continue;
	    ...
	    //~ }
        //~ console.log(minSkill +' '+ scene.minh);
        //~ console.log(scene.minh-minSkill);
        var maxs = 2*maxSkill/(2-(1-maxSkill));
        miny = (scene.minh-minSkill/2)/(maxs-minSkill/2)*char_scenes.length*text_height;
        maxy = (scene.maxh-minSkill/2)/(maxs-minSkill/2)*char_scenes.length*text_height;
        scene.height = maxy-miny-5;
        avg = miny+(maxy-miny)/2;
	    scene.y = avg - scene.height/2.0;

	    if (equal_scenes) {
		scene.x = scene.start+panel_shift;
	    } else {
		scene.x = (scene.start*scene.duration+panel_shift)*panel_width;
        //console.log("sc"+scene.start+" "+panel_shift+" "+scene.duration+" "+panel_width);
	...
	if (scene.first_scene != null) { // i.e. if it's a char scene node
		...
		scene.x = panel_shift*duration_spacing;//scene.first_scene.x - name_shift;
	    //~ else
...
function calculate_link_positions(scenes, chars, groups, char_map) {    
	...
	scene.in_links.sort(function(a, b) { return a.char_ptr.skills[a.to.start] - b.char_ptr.skills[b.to.start]; });
	scene.out_links.sort(function(a, b) { return a.char_ptr.skills[a.from.start] - b.char_ptr.skills[b.from.start]; });
	...
	for (var i = 0; i < scene.in_links.length; i++) {
	    ...
	    // co-ordinates for the last point on the link path
	    //~ scene.in_links[i].y1 = scene.y + i*(link_width+link_gap) + link_width/2.0;
        //~ console.log(scene.in_links[i]);
        
        if (scene.char_node){
        scene.in_links[i].x1 = scene.x;
        scene.in_links[i].y1 = scene.y;
	        
        }else {
            var spacing=link_width+link_gap;
            if (spacing*scene.in_links.length>scene.height) { spacing = scene.height/scene.in_links.length; }
        scene.in_links[i].y1 = scene.y  + (scene.height-scene.in_links.length*spacing)/2 + i*spacing;// + link_width/2.0
	    //~ scene.in_links[i].y1 = scene.in_links[i].char_ptr.skills[scene.in_links[i].to.start]*chars.length*text_height;
...
function draw_nodes(scenes, svg, chart_width, chart_height, folder, safe_name) {
    ...
    .enter().append("g")
      .attr("class", "node")
      .attr("transform", function(d) { return "translate(" + d.x/2 + "," + d.y + ")"; })
    ...
    node.append("rect")
      ...
      //~ .attr("ry", 10)
      //~ .attr("fill", "#fff")
      //~ .attr("fill", function(d) {return color(d.maxh)})
      .attr("fill", "#1f77b4")
      .style("opacity", 0.6)
    .append("title")
    ...
    if (name_bg) {
      node.append("rect")
	.filter(function(d) {
	    return d.char_node && d.last_scene==null;
	})
	.attr("x", function(d) { 
    ...
    node.append("rect")
	.filter(function(d) {
	    return d.char_node && d.first_scene==null;
	})
	//~ .attr("x", function(d) { 
	    //~ return +((d.name.length+2)*5); 
	//~ })
	...
	.attr("transform", function(d) { return "translate(7," + (d.height/2-20) + ") rotate(90)";})
        //.attr("background", "#fff")
	.text(function(d) { return d.minh + " - "+d.maxh; })
        .style("fill", "#000")
        //~ .style("stroke", "#fff")
    ...
    //~ var devent =  Math.round(d3.event.y/50)*50-d.height/2;
    //~ console.log(devent)
    //~ console.log(d3.event.x)
	//~ var newy = Math.max(0, Math.min(chart_height - d.height, devent));
	var ydisp = d.y - newy;
	d3.select(this).attr("transform", "translate(" 
			     + (d.x = Math.max(0, Math.min(chart_width - d.width, d3.event.x))) + "," 
			     //~ + d.x  + "," 
			     + (d.y = Math.max(0, Math.min(chart_height - d.height, d3.event.y))) + ")");
			     //~ + (d.y = Math.max(0, Math.min(chart_height - d.height,  devent))) + ")");
...
function draw_links(links, svg, safe_name) {
	...
	//~ .style("stroke", function(d) { return d3.rgb(color(d.char_ptr.trajectory)).toString(); })
	.style("stroke", function(d) { return color[d.char_ptr.trajectory]; })
	  //~ .style("stroke-width", link_width)
    .style("stroke-width",  function(d) {if (d.char_ptr.trajectory>0) { return link_width*1.5;} else {return link_width;}})
          .style("stroke-opacity", function(d) {if (d.char_ptr.trajectory>0) { return d.char_ptr.trajectory;} else {return "0.3";}})
          //~ .style("stroke-opacity", function(d) {return d.trajectory;})
	...
	d3.selectAll("[charid=\"" + d.from.comic_name + "_" + d.char_id + "\"]")
	    .style("stroke-width",  function(d) {if (d.char_ptr.trajectory>0) { return link_width*2.5;} else {return link_width*2;}})
...
function draw_chart(name, safe_name, folder, tie_breaker, center_sort, collapse,start_year) {
    ...
    var minSkill = parseFloat(j['minskill']);
    var maxSkill = parseFloat(j['maxskill']);
    console.log()
	...
	for (var i = 0; i < jscenes.length; i++) {
	    ...
	    var minh = parseFloat(jscenes[i]['min']);
	    var maxh = parseFloat(jscenes[i]['max']);
	    var start;
	    if (equal_scenes) {
	    ...
	    scenes[scenes.length] = new SceneNode(jscenes[i]['chars'], 
						  start, duration, 
						  parseInt(jscenes[i]['id']),minh,maxh);
	    scenes[scenes.length-1].comic_name = safe_name;
	    //~ total_panels += duration;
	    total_panels = Math.max(start*duration,total_panels);
	} // for
    //~ console.log('total_panels' + total_panels);
	scenes.sort(function(a, b) { return a.start - b.start; });
	...
	var panel_width = (width-longest_name)/(total_panels+3*duration_spacing);
	var panel_shift = panel_width; //Math.round(longest_name/panel_width);
	//~ var panel_shift = Math.round(longest_name/panel_width);
	//~ total_panels += panel_shift;
    //~ console.log('total_panels' + total_panels)
    //~ console.log(width/total_panels)
	...
	d3.xml(folder + ".xml", function(x) {
       ...
       // console.log('new char ' + xchars[i].name);
		chars[chars.length] = new Character_(xchars[i].name, xchars[i].id, xchars[i].group, xchars[i].order, xchars[i].skills, xchars[i].trajectory);
	    ...
	    char_scenes.forEach(function(cs) {
			...
			cs.y = character.skillorder*text_height;
		    }
	    });
   ...
   // console.log("cs "+panel_shift+" "+total_panels)
    for (var i = 0; i < total_panels/duration_spacing+1; i++) {
    	//console.log("x"+margin.left+6+(i*duration_spacing+panel_shift)*panel_width);
    var svgyear = svg.append("text")
        .attr("x",
        		function(){
        	console.log(margin.left+6+(i*duration_spacing+panel_shift)*panel_width/2)
        	return margin.left+6+(i*duration_spacing+panel_shift)*panel_width/2}) //scene.start*scene.duration+panel_shift)*panel_width
		.attr("y", 0)
        .attr("fill", "#000")
...
dir = ".";
draw_chart("NFL Offensive Skills", "nfl_off_skills", "resources/json/nfl-chart/NFL0", true, true, true,2000);
//draw_chart("NBA Defensive Skills", "nba_def_skills", "resources/json/nfl-chart/NBA1", true, true, false,2002);
//draw_chart("NBA Offensive Skills", "nba_off_skills", "resources/json/nfl-chart/NBA0", true, true, false,2002);
