  ...
  $(function() {
    ...
    draw_movies = function() {
      ...
      movies.transition().duration(1000).attr("transform", function(d) {
        return "translate(" + (x_scale(d["profit"])) + "," + (y_scale(d["rottenTomatoes"])) + ")";
      });
    ...
    render_vis = function(csv) {
      ...
      vis.append("text").attr("x", h / 2).attr("y", 20).attr("text-anchor", "middle").attr("class", "axisTitle").attr("transform", "rotate(270)scale(-1,1)translate(" + pb + "," + 0 + ")").text("Rating (rottenTomatoes %)");
    ...
    show_details = function(movie_data, index, element) {
      ...
      box.y = Math.round(y_scale_reverse(movie_data["rottenTomatoes"])) - (r_scale(movie_data["budget"]) + pt + -55);
      tooltipWidth = parseInt(d3.select('#tooltip').style('width').split('px').join(''));
      msg = '<p class="title">' + movie_data["film"] + '</p>';
      msg += '<table>';
      msg += '<tr><td>Rating:</td><td>' + movie_data["rottenTomatoes"] + '%</td></tr>';
      msg += '<tr><td>Budget:</td><td>' + movie_data["budget"] + ' mil</td></tr>';
      msg += '<tr><td>WorldwideGross:</td><td>' + movie_data["worldwideGross"] + ' mil</td></tr>';
      ...
      crosshairs_g.append("line").attr("class", "crosshair").attr("x1", 0 + 3).attr("x2", x_scale(movie_data["profit"]) - r_scale(movie_data["budget"])).attr("y1", y_scale(movie_data["rottenTomatoes"])).attr("y2", y_scale(movie_data["rottenTomatoes"])).attr("stroke-width", 1);
      return crosshairs_g.append("line").attr("class", "crosshair").attr("x1", x_scale(movie_data["profit"])).attr("x2", x_scale(movie_data["profit"])).attr("y1", 0 + 3).attr("y2", y_scale(movie_data["rottenTomatoes"]) - r_scale(movie_data["budget"])).attr("stroke-width", 1);
    };
    ...
    d3.json("../bubble-chart3-data", render_vis);
    update = function() {
      update_data();
      draw_movies();
      return draw_details();
